...
const dom = {
  ...
  openFavoriteBtn: $("#openFavoriteBtn"),
...
function init() {
  ...
  if (dom.openFavoriteBtn) {
    dom.openFavoriteBtn.addEventListener("click", () => {
      renderFavorites();
    });
  }
...
function initBottomNav() {
  ...
  navItems.forEach((item) => {
    item.addEventListener("click", (event) => {
      ...
      if (target === "community") {
        showScreen("search", "community");
        return;
      }

      if (target === "live") {
        showScreen("search", "live");
        return;
      }
...
function renderFavorites(move = true) {
  if (move) showScreen("favorites", "search");
